...
static void handle_interrupts(void* context)
{
    ...
    *edge_capture_ptr = IORD_ALTERA_AVALON_PIO_EDGE_CAP(GPI0_BTN_BASE);

    //Write to edge capture register to reset it
    IOWR_ALTERA_AVALON_PIO_EDGE_CAP(GPI0_BTN_BASE,1<<0);

    /* Read the PIO to delay ISR exit. This is done to prevent a
    spurious interrupt in systems with high processor -> pio
    latency and fast interrupts. */
    IORD_ALTERA_AVALON_PIO_EDGE_CAP(GPI0_BTN_BASE);
}
...
static void init_interrupt_pio()
{
    ...
    IOWR_ALTERA_AVALON_PIO_IRQ_MASK(GPI0_BTN_BASE,1<<0);
    ...
    alt_ic_isr_register(GPI0_BTN_IRQ_INTERRUPT_CONTROLLER_ID,
    		GPI0_BTN_IRQ, handle_interrupts, edge_capture_ptr, 0x0);
...
int main(void)
{
    ...
    printf("\nHello from NIOS-V on DE0-NANO: booting from EPCS and running from internal TCMEM!\n");
